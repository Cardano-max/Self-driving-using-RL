...
void loop() {
  ...
  // If right sensor detects the black line, then turn right
  ...
  else if (outerrightIRSensorValue == LOW && innerrightIRSensorValue == LOW && midIRSensorValue == LOW && innerleftIRSensorValue == HIGH && outerleftIRSensorValue == HIGH) {
    currentstate = rightjunction;
    ...
    if (currentstate != previousstate) {
      ...
      if (midIRSensorValue == LOW && outerrightIRSensorValue == LOW && innerrightIRSensorValue == LOW && innerleftIRSensorValue == LOW && outerleftIRSensorValue == LOW) {
        ...
        Serial.print("doublelineright: ");
        Serial.println(doublelineright);

        if (doublelineright >= 1 && doublelineright < 2) {
          currentstate = straight;
          rotateMotor(defaultspeed, defaultspeed);
          previousstate = currentstate;
          delay(400);
        ...
        else if (doublelineright == 4) {
          doublelineright++;
        }

        else if (doublelineright == 5) {
          currentstate = rightjunction;
          rotateMotor(0, 0);
          delay(2000);
          rotateMotor(-sharpturnspeed, sharpturnspeed);
  ...
  else if (outerrightIRSensorValue == LOW && innerrightIRSensorValue == LOW && midIRSensorValue == LOW && innerleftIRSensorValue == LOW && outerleftIRSensorValue == HIGH) {
    currentstate = rightjunction;
